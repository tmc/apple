...
//
// See: https://developer.apple.com/documentation/AppKit/NSComboBoxDelegate
...
func NSComboBoxDelegateObjectFromID(id objc.ID) NSComboBoxDelegateObject {
	return NSComboBoxDelegateObject{
...
//
// [Apple Documentation]: https://developer.apple.com/documentation/appkit/nscomboboxdelegate
type NSComboBoxDelegateConfig struct {

	// Manipulating the selection
	// SelectionDidChange — Informs the delegate that the pop-up list selection has finished changing.
	SelectionDidChange func(notification foundation.NSNotification)
	// SelectionIsChanging — Informs the delegate that the pop-up list selection is changing.
	SelectionIsChanging func(notification foundation.NSNotification)
	// WillDismiss — Informs the delegate that the pop-up list is about to be dismissed.
	WillDismiss func(notification foundation.NSNotification)
	// WillPopUp — Informs the delegate that the pop-up list is about to be displayed.
	WillPopUp func(notification foundation.NSNotification)
}

// NewNSComboBoxDelegate creates an Objective-C object implementing the [NSComboBoxDelegate] protocol.
//
// Each call registers a unique Objective-C class containing only the methods
// set in config. This means [NSObject.RespondsToSelector] works correctly
// for optional delegate methods — only non-nil callbacks are registered.
//
// The returned [NSComboBoxDelegateObject] satisfies the [NSComboBoxDelegate] interface
// and can be passed directly to SetDelegate and similar methods.
//
// See [Apple Documentation] for protocol details.
//
// [Apple Documentation]: https://developer.apple.com/documentation/appkit/nscomboboxdelegate
func NewNSComboBoxDelegate(config NSComboBoxDelegateConfig) NSComboBoxDelegateObject {
	n := delegateClassCounter.Add(1)
	className := fmt.Sprintf("GoNSComboBoxDelegate_%d", n)
	...
	if config.WillDismiss != nil {
		fn := config.WillDismiss
		methods = append(methods, objc.MethodDef{
	...
	proto := objc.GetProtocol("NSComboBoxDelegate")
	...
	if err != nil {
		panic(fmt.Sprintf("NewNSComboBoxDelegate: RegisterClass %s: %v", className, err))
	}
