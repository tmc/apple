...
func (nc NSMatrixClass) Class() objc.Class {
	return nc.class
}
...
// # Return Value
//
// true if the point lies within one of the cells in the receiver; false if
// the point falls outside the bounds of the receiver or lies within an
// intercell spacing.
//
...
// # Return Value
//
// true if the receiver contains a cell whose tag matches `anInt`, or false if
// no such cell exists
//
// # Discussion
//
...
// `flag` is false, the [NSControl] method [calcSize()] will be invoked before
// any further drawing is done.
//
...
// sending the message for the remaining cells. If it returns true, [NSMatrix]
// proceeds to the next cell.
//
// object: The object that is sent the selector for each cell in the matrix.
//
// flag: true if the method should iterate through all cells in the matrix; false if
// it should iterate through just the selected cells in the matrix.
//
// # Discussion
//
...
func (m NSMatrix) ViewStringForToolTipPointUserData(view INSView, tag NSToolTipTag, point corefoundation.CGPoint, data unsafe.Pointer) string {
...
// false, selection in the matrix is on a row-by-row basis. The default value
...
// selector, this property also sets `ignoresMultiClick` to false; otherwise,
