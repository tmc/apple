...
func (nc NSSoundClass) Class() objc.Class {
	return nc.class
}
...
//
// byRef: When true only the name of the sound is stored with the [NSSound] instance
// when archived using [encode(with:)]; otherwise the audio data is archived
...
// # Return Value
//
// true when playback is paused successfully, false when playback is already
// paused or when an error occurred.
//
...
// # Return Value
//
// true when playback is initiated, false when playback is already in progress
...
// # Return Value
//
// true when playback is resumed, false when playback is in progress or when
// an error occurred.
//
// # Discussion
//
...
// # Discussion
//
// When the value of this property is true, the sounds restarts playback when
// it finishes and does not send [SoundDidFinishPlaying] to its delegate when
// it reaches the end of its content and restarts playback. The default value
