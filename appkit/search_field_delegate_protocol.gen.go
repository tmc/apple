...
//
// See: https://developer.apple.com/documentation/AppKit/NSSearchFieldDelegate
...
func (o NSSearchFieldDelegateObject) BaseObject() objectivec.Object {
	return o.Object
}
...
func NSSearchFieldDelegateObjectFromID(id objc.ID) NSSearchFieldDelegateObject {
	return NSSearchFieldDelegateObject{
...
//
// [Apple Documentation]: https://developer.apple.com/documentation/appkit/nssearchfielddelegate
type NSSearchFieldDelegateConfig struct {

	// Detecting the Start and End of a Search
	// SearchFieldDidStartSearching — The method that is called when the search field begins searching for content.
	SearchFieldDidStartSearching func(sender NSSearchField)
	// SearchFieldDidEndSearching — The method that is called when the search field has ended its search for content.
	SearchFieldDidEndSearching func(sender NSSearchField)
}

// NewNSSearchFieldDelegate creates an Objective-C object implementing the [NSSearchFieldDelegate] protocol.
//
// Each call registers a unique Objective-C class containing only the methods
// set in config. This means [NSObject.RespondsToSelector] works correctly
// for optional delegate methods — only non-nil callbacks are registered.
//
// The returned [NSSearchFieldDelegateObject] satisfies the [NSSearchFieldDelegate] interface
// and can be passed directly to SetDelegate and similar methods.
//
// See [Apple Documentation] for protocol details.
//
// [Apple Documentation]: https://developer.apple.com/documentation/appkit/nssearchfielddelegate
func NewNSSearchFieldDelegate(config NSSearchFieldDelegateConfig) NSSearchFieldDelegateObject {
	n := delegateClassCounter.Add(1)
	className := fmt.Sprintf("GoNSSearchFieldDelegate_%d", n)

	var methods []objc.MethodDef

	if config.SearchFieldDidStartSearching != nil {
		fn := config.SearchFieldDidStartSearching
	...
	if config.SearchFieldDidEndSearching != nil {
		fn := config.SearchFieldDidEndSearching
		methods = append(methods, objc.MethodDef{
	...
	proto := objc.GetProtocol("NSSearchFieldDelegate")
	...
	if err != nil {
		panic(fmt.Sprintf("NewNSSearchFieldDelegate: RegisterClass %s: %v", className, err))
	}
