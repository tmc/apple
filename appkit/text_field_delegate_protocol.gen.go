...
//
// See: https://developer.apple.com/documentation/AppKit/NSTextFieldDelegate
...
func (o NSTextFieldDelegateObject) BaseObject() objectivec.Object {
	return o.Object
}
...
func NSTextFieldDelegateObjectFromID(id objc.ID) NSTextFieldDelegateObject {
	return NSTextFieldDelegateObject{
...
//
// [Apple Documentation]: https://developer.apple.com/documentation/appkit/nstextfielddelegate
type NSTextFieldDelegateConfig struct {

	// Controlling Editing Behavior
	// TextViewCandidatesForSelectedRange — Provides a customized list of candidates to the text view’s `candidateListTouchBarItem`. This method returns an array of objects that represent the elements of a selection.
	TextViewCandidatesForSelectedRange func(textField NSTextField, textView NSTextView, selectedRange foundation.NSRange) foundation.INSArray

	// Other Methods
	// TextViewShouldSelectCandidateAtIndex — Notifies the delegate that the user selected the candidate at index in `-[NSCandidateListTouchBarItem candidates]` for the text view’s `candidateListTouchBarItem`. Returns a Boolean value that indicates whether to select the text object at the index.
	TextViewShouldSelectCandidateAtIndex func(textField NSTextField, textView NSTextView, index uint) bool
}

// NewNSTextFieldDelegate creates an Objective-C object implementing the [NSTextFieldDelegate] protocol.
//
// Each call registers a unique Objective-C class containing only the methods
// set in config. This means [NSObject.RespondsToSelector] works correctly
// for optional delegate methods — only non-nil callbacks are registered.
//
// The returned [NSTextFieldDelegateObject] satisfies the [NSTextFieldDelegate] interface
// and can be passed directly to SetDelegate and similar methods.
//
// See [Apple Documentation] for protocol details.
//
// [Apple Documentation]: https://developer.apple.com/documentation/appkit/nstextfielddelegate
func NewNSTextFieldDelegate(config NSTextFieldDelegateConfig) NSTextFieldDelegateObject {
	n := delegateClassCounter.Add(1)
	className := fmt.Sprintf("GoNSTextFieldDelegate_%d", n)

	var methods []objc.MethodDef

	if config.TextViewCandidatesForSelectedRange != nil {
		fn := config.TextViewCandidatesForSelectedRange
	...
	if config.TextViewShouldSelectCandidateAtIndex != nil {
		fn := config.TextViewShouldSelectCandidateAtIndex
		methods = append(methods, objc.MethodDef{
	...
	proto := objc.GetProtocol("NSTextFieldDelegate")
	...
	if err != nil {
		panic(fmt.Sprintf("NewNSTextFieldDelegate: RegisterClass %s: %v", className, err))
	}
