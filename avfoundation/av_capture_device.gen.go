...
func (ac AVCaptureDeviceClass) Class() objc.Class {
	return ac.class
}
...
// [AVCapturePrimaryConstituentDeviceSwitchingBehaviorRestricted] requires
...
// # Discussion
//
// This method sets the torch mode to [AVCaptureTorchModeOn] and sets the
// level to the specified value. If the device doesn’t support this mode or
// if you specify a value for `torchLevel` that’s outside the accepted
// range, this method raises an exception. If the torch value is within the
// accepted range but greater than the currently supported maximum—perhaps
...
// (front- or back-facing). Pass [AVCaptureDevicePositionUnspecified] to
// search for devices regardless of position.
//
// # Return Value
//
...
// default authorization status is [AVAuthorizationStatusNotDetermined], which
...
func (_AVCaptureDeviceClass AVCaptureDeviceClass) RequestAccessForMediaTypeCompletionHandler(mediaType AVMediaType, handler BoolHandler) {
	_block1, _ := NewBoolBlock(handler)
...
// [AVCapturePrimaryConstituentDeviceSwitchingBehaviorAuto] for devices that
// support camera switching, and
// [AVCapturePrimaryConstituentDeviceSwitchingBehaviorUnsupported] for those
// that don’t.
//
// This property is key-value observable.
//
...
// # Discussion
//
// When the value is true, the system artificially lights the subject’s face
...
// [AVCaptureCenterStageControlModeUser], results in the system throwing an
// exception.
//
