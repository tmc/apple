...
// [AVCapturePhotoOutput.CapturePhotoWithSettingsDelegate] method, you
// describe the settings for that capture request in an
// [AVCapturePhotoSettings] object. When the capture begins, the photo output
// calls your delegate methods and provides an
// [AVCaptureResolvedPhotoSettings] object detailing the settings that are in
// effect for that capture. Resolved photo settings objects are immutable;
// they describe a request that has already been made.
//
// The [AVCaptureResolvedPhotoSettings.UniqueID] property of a resolved photo
// settings object passed to one of your [AVCapturePhotoCaptureDelegate]
// methods matches the [AVCapturePhotoSettings.UniqueID] value of the
// [AVCapturePhotoSettings] object you passed when requesting capture. Use
// this value to determine which delegate method calls correspond to which
// capture requests.
//
// Some photo capture settings are automatic, such as the
// [AVCapturePhotoSettings.FlashMode] property. For such settings, the photo
// output determines whether to use that feature at the moment of
// capture—you don’t know when requesting a capture whether the feature is
// active when the capture completes. When the photo output calls your
// delegate methods, the provided [AVCaptureResolvedPhotoSettings] object
// details which automatic features have been set for that capture.
//
...
// the [AVCapturePhotoSettings.PreviewPhotoFormat] setting, the photo output
// chooses dimensions that best match your requested size while preserving the
// aspect ratio of the captured photo. When the photo output calls your
// delegate methods, use the
// [AVCaptureResolvedPhotoSettings.PreviewDimensions] property of the resolved
// settings to find the actual preview image dimensions. See the methods
// listed in Examining Output Dimensions for other cases where output
// dimensions can change at capture time.
//
...
// # Discussion
//
// The value of this property matches the matches the
// [AVCapturePhotoSettings.UniqueID] value of the [AVCapturePhotoSettings]
// object you passed when initiating a photo capture with the
// [AVCapturePhotoOutput.CapturePhotoWithSettingsDelegate] method. Use this
// value to determine which delegate method calls correspond to which capture
// requests.
//
...
// photo count is `6`.
//
// The [AVCapturePhoto.PhotoCount] property of each [AVCapturePhoto] object
// delivered to your delegate indicates where that capture result relates to
// this sequence. When your delegate receives a photo whose
// [AVCapturePhoto.PhotoCount] value matches the
// [AVCaptureResolvedPhotoSettings.ExpectedPhotoCount], you know you’ve
// received the last one for the given capture request.
//
