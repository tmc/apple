...
//
// outError: If the status of the value for the key is [KeyValueStatusFailed], upon
...
//
// metadataItem: The metadata item that provides the [Identifier], [ExtendedLanguageTag],
// and other property values that you want the newly created metadata item to
// share. The system ignores the value of this template metadata item.
//
...
// # Discussion
// 
// Use this method to create metadata items for optional display purposes. For
// example, a metadata item that provides a large image, should only load the
// image when the system requests it.
// 
// When you load the [value] of a metadata item you created with this
// initializer, the closure you provide as the value loading handler executes
// on an arbitrary dispatch queue, off the main thread. The handler can
// perform I/O and other necessary operations to obtain the value. If loading
// of the value succeeds, provide the value by calling [RespondWithValue]. If
// loading of the value fails, provide an instance an error object that
// describes the failure by calling [RespondWithError].
//
// [value]: https://developer.apple.com/documentation/AVFoundation/AVPartialAsyncProperty/value
//
