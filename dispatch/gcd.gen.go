...
var (
	_dispatch_io_read_f  func(channel uintptr, offset int64, length uint64, queue uintptr, ctx uintptr, handler uintptr)
	_dispatch_io_write_f func(channel uintptr, offset int64, data uintptr, queue uintptr, ctx uintptr, handler uintptr)
	_dispatch_walltime   func(when *syscall.Timespec, delta int64) uint64
)

// _dispatch_data_apply_f is the function-pointer variant of dispatch_data_apply.
var _dispatch_data_apply_f func(data uintptr, ctx uintptr, applier uintptr) bool
...
// sourceWorkMap stores all handlers for a dispatch source under its single
// context ID. Entries remain until the source's cancel handler runs.
var sourceWorkMap sync.Map // id (uintptr) → *sourceHandlers
var sourceWorkCounter atomic.Uint64
var sourceContextMu sync.Mutex
...
// newIOCleanupBlock wraps a Go cleanup handler in an ObjC block for
// dispatch_io_create's cleanup_handler parameter, which takes a block, not a
// function pointer. The caller must Release the block after passing it to
// the create call (libdispatch copies it).
func newIOCleanupBlock(cleanup func(error)) objc.Block {
	return objc.NewBlock(func(_ objc.Block, errCode int32) {
		var goErr error
		if errCode != 0 {
...
func init() {
	...
	sharedPersistentTrampoline = purego.NewCallback(func(ctx uintptr) {
		if value, ok := sourceWorkMap.Load(ctx); ok {
			handlers := value.(*sourceHandlers)
			handlers.mu.RLock()
			fn := handlers.event
			handlers.mu.RUnlock()
			if fn != nil {
	...
	sharedCancelTrampoline = purego.NewCallback(func(ctx uintptr) {
		if value, ok := sourceWorkMap.Load(ctx); ok {
			...
			handlers.mu.Unlock()
			defer sourceWorkMap.Delete(ctx)
			if fn != nil {
				fn()
			}
		}
	...
	if lib == 0 {
		...
		_dispatch_io_read_f = func(uintptr, int64, uint64, uintptr, uintptr, uintptr) {
			panic("dispatch: dispatch_io_read_f unavailable")
		}
		_dispatch_io_write_f = func(uintptr, int64, uintptr, uintptr, uintptr, uintptr) {
	...
	purego.RegisterLibFunc(&_dispatch_walltime, lib, "dispatch_walltime")
...
// dispatchGenerationSentinels keeps the raw dispatch_once entry points out of
// functions.gen.go. Go callers use sync.Once. These are fields, not bindings.
type dispatchGenerationSentinels struct {
	_dispatch_once   func(predicate *int, block unsafe.Pointer)
	_dispatch_once_f func(predicate *int, context unsafe.Pointer, function unsafe.Pointer)
}
...
// dispatch_time_t suppresses the generated C-name typedef from the hand-shaped
// API. Time is the public spelling below.
type dispatch_time_t = uint64

// Time is an abstract dispatch deadline or timeout.
type Time uint64
...
const (
	...
	TimeForever Time = ^Time(0)
)

// QueueAttr configures queue creation.
//
// attr holds the opaque dispatch_queue_attr_t handle. It is a uintptr rather
// than an unsafe.Pointer because dispatch_queue_attr_t is an opaque handle,
// not a pointer into Go-visible memory, and the generated attribute functions
// take and return it by value.
type QueueAttr struct {
	attr uintptr
}

func (a QueueAttr) raw() dispatch_queue_attr_t {
	return dispatch_queue_attr_t(a.attr)
}
...
func ConcurrentQueueAttr() QueueAttr {
	return QueueAttr{attr: uintptr(_dispatch_queue_attr_concurrent)}
}
...
func (a QueueAttr) WithQOS(qos QOS, relativePriority int) (QueueAttr, error) {
	attr := _dispatch_queue_attr_make_with_qos_class(a.attr, uint32(qos), int32(relativePriority))
	if attr == 0 {
...
//
// Dispatch queues created by this package are never released. Create them
// once and reuse them.
type Queue struct {
...
//
// Dispatch groups created by this package are never released. Create them
// once and reuse them.
type Group struct {
...
//
// Dispatch semaphores created by this package are never released. Create them
// once and reuse them.
...
// Work submitted to this queue is delivered only while the process main thread
// runs the dispatch event loop. In a Go program, call Main from the main
// goroutine after locking it with runtime.LockOSThread.
func MainQueue() Queue {
	return Queue{queue: dispatch_queue_t(_dispatch_main_q)}
}

// Main enters the main dispatch event loop and does not return. Call Main from
// the main goroutine after runtime.LockOSThread so delivery remains on the
// process main thread.
func Main() {
	_dispatch_main()
}
...
//
// The wait happens on the Go side rather than in dispatch_async_and_wait:
// that C primitive parks the caller in a libdispatch ownership-handoff wait
// that loses its wakeup when Go's async-preemption signal (SIGURG) lands in
// the handoff window, deadlocking the caller when the queue has a pending
// async item. Waiting on a Go channel is signal-safe. The observable
// difference is that the work always runs on a GCD worker thread instead of
// occasionally being executed inline on the calling thread.
...
// QueueFromHandle wraps a borrowed queue handle. The caller retains ownership;
// the wrapper never releases it.
...
//
// Like AsyncAndWait, this avoids dispatch_async_and_wait, whose
// ownership-handoff wait loses its wakeup when Go's async-preemption signal
// (SIGURG) interrupts it. The item is submitted asynchronously and awaited
// with dispatch_block_wait, whose wait path tolerates interruption.
func (q Queue) AsyncAndWaitWorkItem(item WorkItem) {
	_dispatch_async(uintptr(q.queue), unsafe.Pointer(item.block))
	_dispatch_block_wait(unsafe.Pointer(item.block), uint64(TimeForever))
}
...
func After(when Time, queue Queue, work func()) {
...
func TimeFromNow(nanoseconds int64) Time {
	return Time(_dispatch_time(uint64(TimeNow), nanoseconds))
}

// WalltimeFromDate returns a dispatch time for an absolute wall-clock date.
func WalltimeFromDate(date time.Time) Time {
	when := syscall.Timespec{Sec: date.Unix(), Nsec: int64(date.Nanosecond())}
	return Time(_dispatch_walltime(&when, 0))
}
...
func WorkItemCreate(flags WorkItemFlags, work func()) WorkItem {
	...
	item := WorkItem{block: objc.Block(_dispatch_block_create(uint64(flags), unsafe.Pointer(block)))}
	block.Release()
	return item
}
...
func WorkItemCreateWithQOS(flags WorkItemFlags, qos QOS, relativePriority int, work func()) WorkItem {
	...
	item := WorkItem{block: objc.Block(_dispatch_block_create_with_qos_class(uint64(flags), uint32(qos), int32(relativePriority), unsafe.Pointer(block)))}
	block.Release()
	return item
}
...
func (w WorkItem) Wait(timeout Time) bool {
...
func (g Group) Wait(timeout Time) bool {
...
func (s Semaphore) Wait(timeout Time) bool {
...
// SemaphoreFromHandle wraps a borrowed semaphore handle. The caller retains
...
// GroupFromHandle wraps a borrowed group handle. The caller retains ownership;
// the wrapper never releases it.
...
// DataFromHandle wraps a borrowed data handle. The caller retains ownership;
// do not call Release on the borrowed wrapper.
...
func (d Data) Apply(fn func(region Data, offset int, buf []byte) bool) bool {
	...
	return _dispatch_data_apply_f(uintptr(d.data), id, sharedApplyTrampoline)
}
...
//
// A Source must be Cancelled when done so its Go handlers can be released.
// Call Release after cancellation has completed to release the dispatch object.
type Source struct {
	source  dispatch_source_t
	context uintptr
}

// SourceFromHandle wraps a borrowed source handle. The caller retains
// ownership; do not call Release on the borrowed wrapper.
...
func SourceCreate(typ SourceType, handle, mask uintptr, queue Queue) Source {
	source := _dispatch_source_create(uintptr(typ), handle, mask, uintptr(queue.queue))
	context := uintptr(sourceWorkCounter.Add(1))
	_dispatch_set_context(source, context)
	_dispatch_source_set_cancel_handler_f(source, unsafe.Pointer(sharedCancelTrampoline))
	return Source{source: dispatch_source_t(source), context: context}
}

func sourceHandlersFor(s Source) *sourceHandlers {
	context := s.context
	if context == 0 {
		sourceContextMu.Lock()
		context = _dispatch_get_context(uintptr(s.source))
		if _, ok := sourceWorkMap.Load(context); !ok {
			context = uintptr(sourceWorkCounter.Add(1))
			_dispatch_set_context(uintptr(s.source), context)
			_dispatch_source_set_cancel_handler_f(uintptr(s.source), unsafe.Pointer(sharedCancelTrampoline))
		}
		sourceContextMu.Unlock()
	}
	value, _ := sourceWorkMap.LoadOrStore(context, new(sourceHandlers))
	return value.(*sourceHandlers)
}
...
// Every Source created by this package must be cancelled when done so its
// event, cancel, and registration handlers can be released.
func (s Source) Cancel() {
	_dispatch_source_cancel(uintptr(s.source))
}

// Release releases the underlying dispatch source. Cancel the source and wait
// for cancellation to complete before calling Release.
func (s Source) Release() {
	if s.source != 0 {
		_dispatch_release(uintptr(s.source))
	}
}
...
func (s Source) SetTimer(start Time, interval, leeway time.Duration) {
...
// IO wraps a dispatch I/O channel. Close and Release channels created by this
// package when they are no longer needed.
type IO struct {
	io dispatch_io_t
}

// IOFromHandle wraps a borrowed I/O handle. The caller retains ownership; do
// not call Release on the borrowed wrapper.
...
// libdispatch controls the descriptor until it calls cleanup. The caller must
// not close or modify the descriptor before then; closing it earlier traps in
// libdispatch, not in Go. The caller remains responsible for closing the
// descriptor after cleanup runs.
func IOCreate(typ IOType, fd int, queue Queue, cleanup func(err error)) IO {
	block := newIOCleanupBlock(cleanup)
	defer block.Release()
	h := _dispatch_io_create(uint(typ), int32(fd), uintptr(queue.queue), uintptr(block))
...
func IOCreateWithPath(typ IOType, path string, oflag int, mode uint32, queue Queue, cleanup func(err error)) IO {
	block := newIOCleanupBlock(cleanup)
	defer block.Release()
	h := _dispatch_io_create_with_path(uint(typ), path, int32(oflag), uint16(mode), uintptr(queue.queue), uintptr(block))
	return IO{io: dispatch_io_t(h)}
}
...
func IOCreateWithIO(typ IOType, parent IO, queue Queue, cleanup func(err error)) IO {
	block := newIOCleanupBlock(cleanup)
	defer block.Release()
	h := _dispatch_io_create_with_io(uint(typ), uintptr(parent.io), uintptr(queue.queue), uintptr(block))
	return IO{io: dispatch_io_t(h)}
}
...
func (i IO) Read(offset int64, length uint64, queue Queue, handler func(done bool, data Data, err error)) {
	...
	_dispatch_io_read_f(uintptr(i.io), offset, length, uintptr(queue.queue), id, sharedIOTrampoline)
}
...
func (i IO) Write(offset int64, data Data, queue Queue, handler func(done bool, remaining Data, err error)) {
	id := storeIOWork(handler)
	_dispatch_io_write_f(uintptr(i.io), offset, uintptr(data.data), uintptr(queue.queue), id, sharedIOTrampoline)
}

// Close closes the IO channel. Pass IOStop to cancel outstanding operations.
// A final operation callback does not transfer control of an IOCreate file
// descriptor back to the caller; wait for the channel cleanup handler before
// closing or modifying the descriptor.
func (i IO) Close(flags IOCloseFlags) {
	_dispatch_io_close(uintptr(i.io), uint(flags))
}

// Release releases the underlying dispatch I/O channel. Close the channel
// before calling Release. For channels made by IOCreate, Release is not the
// descriptor-ownership handoff: the cleanup handler is.
func (i IO) Release() {
	if i.io != 0 {
		_dispatch_release(uintptr(i.io))
	}
}
...
func (i IO) Barrier(fn func()) {
	...
	defer block.Release()
	_dispatch_io_barrier(uintptr(i.io), unsafe.Pointer(block))
}
