// Command espressoe5rtstack runs a stack of transformer blocks on the Neural Engine
...
//
//	go run ./examples/ane/espressoe5rtstack
//	go run ./examples/ane/espressoe5rtstack -layers 6 -dim 128 -heads 8 -seq 32 -hidden 256
package main
...
//
// Whether an execution stream keeps an encoded operation's function slot is
// unmeasured here until probeEncodedStreamRetention can run with one more
// function than this process can hold. That probe keeps the stream alive after
// releasing its operations, then compares the next function before and after
...
// probeEncodedStreamRetention asks whether an encoded stream retains a
// function slot after the caller releases its operation handle. It uses the
// next compiled function as a control: it must fail while the stream is live
// and succeed after stream release. A result with no extra function is
// UNMEASURED rather than evidence either way.
func probeEncodedStreamRetention(lib *e5rt.Lib, fns []layerFunctions, budget, nbytes int) (string, error) {
	...
	for _, f := range fns {
		functions = append(functions,
			function{f.attn, []string{"x"}, []string{"out"}},
			function{f.ffn, []string{"x"}, []string{"out"}},
			function{f.add, []string{"a", "b"}, []string{"out"}},
		)
	}
	if budget >= len(functions) {
		return "  stream-retention control: UNMEASURED (this stack has no function beyond the measured budget)", nil
	}

	stream, err := lib.ExecutionStreamCreate()
	if err != nil {
		return "", fmt.Errorf("stream-retention control: create stream: %w", err)
	}
	defer func() {
		if stream != 0 {
			lib.ExecutionStreamRelease(stream)
		}
	}()
	buffer, err := lib.BufferObjectAlloc(uintptr(max((nbytes+63)&^63, 64)), 0)
	if err != nil {
		return "", fmt.Errorf("stream-retention control: allocate buffer: %w", err)
	}
	defer lib.BufferObjectRelease(buffer)

	var operations []uintptr
	defer func() {
		for _, op := range operations {
			lib.OperationRelease(op)
		}
	}()
	for i, function := range functions[:budget] {
		op, err := newOperation(lib, function.handle)
		if err != nil {
			return "", fmt.Errorf("stream-retention control: create operation %d: %w", i, err)
		}
		operations = append(operations, op)
		for _, name := range function.inputs {
			if err := bind(lib, op, name, buffer, true); err != nil {
				return "", fmt.Errorf("stream-retention control: bind input %q on operation %d: %w", name, i, err)
			}
		}
		for _, name := range function.outputs {
			if err := bind(lib, op, name, buffer, false); err != nil {
				return "", fmt.Errorf("stream-retention control: bind output %q on operation %d: %w", name, i, err)
			}
		}
		if err := lib.EncodeOperation(stream, op); err != nil {
			return "", fmt.Errorf("stream-retention control: encode operation %d: %w", i, err)
		}
	}
	for _, op := range operations {
		if err := lib.OperationRelease(op); err != nil {
			return "", fmt.Errorf("stream-retention control: release operation: %w", err)
		}
	}
	operations = nil

	next := functions[budget]
	op, heldErr := newOperation(lib, next.handle)
	if heldErr == nil {
		if err := lib.OperationRelease(op); err != nil {
			return "", fmt.Errorf("stream-retention control: release extra operation: %w", err)
		}
		return "  stream-retention control: operations released while the stream remained live; the next function was accepted, so this stream did not retain the measured slots", nil
	}
	if !strings.Contains(heldErr.Error(), "status 13") {
		return "", fmt.Errorf("stream-retention control: next function failed for an unexpected reason: %w", heldErr)
	}
	if err := lib.ExecutionStreamRelease(stream); err != nil {
		return "", fmt.Errorf("stream-retention control: release stream: %w", err)
	}
	stream = 0
	op, err = newOperation(lib, next.handle)
	if err != nil {
		return "", fmt.Errorf("stream-retention control: next function is still refused after stream release: %w", err)
	}
	if err := lib.OperationRelease(op); err != nil {
		return "", fmt.Errorf("stream-retention control: release retry operation: %w", err)
	}
	return "  stream-retention control: next function was refused after operation release, then accepted after stream release; the encoded stream retained the function slots", nil
}
...
func run(layers, dim, heads, seq, hidden int) error {
	...
	if retention, err := probeEncodedStreamRetention(lib, fns, budget, acts*2); err != nil {
		return err
	} else {
