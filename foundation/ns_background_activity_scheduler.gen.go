...
// 
// - [NSBackgroundActivityScheduler.Repeats]—If set to [true], the activity is rescheduled at the
// specified interval after finishing. - [NSBackgroundActivityScheduler.Interval]—For repeating
// schedulers, the average interval between invocations of the activity. For
// nonrepeating schedulers, `interval` is the suggested interval of time
// between scheduling the activity and the invocation of the activity. -
// [NSBackgroundActivityScheduler.Tolerance]—The amount of time before or after the nominal fire date when
// the activity should be invoked. The nominal fire date is calculated by
// using the interval combined with the previous fire date or the time when
// the activity is started. These two properties create a window in time,
// during which the activity may be scheduled. The system will more
// aggressively schedule the activity as it nears the end of the grace period
// after the nominal fire date. The default value is half the interval. -
// [NSBackgroundActivityScheduler.QualityOfService]—The default value is [NSQualityOfServiceBackground].
// If you upgrade the quality of service above this level, the system
// schedules the activity more aggressively. The default value is the
// recommended value for most activities. For information on quality of
// service, see [Prioritize Work at the Task Level] in [Energy Efficiency
// Guide for Mac Apps].
// 
...
// [BackgroundActivityResultDeferred]. See the following example.
// 
...
// value of [BackgroundActivityResultDeferred]. The system will invoke your
