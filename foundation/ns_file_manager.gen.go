...
func FileManagerFromID(id objc.ID) FileManager {
	return NSFileManager{objectivec.Object{id}}
}
...
// [AllDomainsMask] constant for this parameter.
// //
...
// value [ItemReplacementDirectory] and the `domain` parameter contains the
// value [UserDomainMask].
//
...
// specify [ItemReplacementDirectory] for the `directory` parameter,
// [UserDomainMask] for the `domain` parameter, and a URL for the `url`
...
// system. For example, setting the directory to [ApplicationDirectory], will
// return the Applications directories in the requested domain. There are a
// number of common directories available in the
// [FileManager.SearchPathDirectory], including: [DesktopDirectory],
// [ApplicationSupportDirectory], and many more.
//
// [FileManager.SearchPathDirectory]: https://developer.apple.com/documentation/Foundation/FileManager/SearchPathDirectory
//
...
// [DirectoryEnumerationSkipsHiddenFiles].
//
// # Return Value
// 
...
// of creating the directory in `url`. If [false], this method fails if any of
// the intermediate parent directories does not exist.
// //
...
//
// createIntermediates: If [true], this method creates any nonexistent parent directories as part
// of creating the directory in `path`. If [false], this method fails if any
// of the intermediate parent directories does not exist. This method also
// fails if any of the intermediate path elements corresponds to a file and
// not a directory.
// //
// [false]: https://developer.apple.com/documentation/Swift/false
// [true]: https://developer.apple.com/documentation/Swift/true
...
// method returns [true], or if the delegate does not implement the
// appropriate methods, the file manager proceeds to remove the file or
// directory. If there is an error removing an item, the file manager may also
// call the delegate’s [FileManagerShouldProceedAfterErrorRemovingItemAtURL]
// or [FileManagerShouldProceedAfterErrorRemovingItemAtPath] method to
...
// [FileManagerItemReplacementWithoutDeletingBackupItem] option is provided in
// `options`.
//
// options: The options to use during the replacement. Typically, you pass
// [FileManagerItemReplacementUsingNewMetadataOnly] for this parameter, which
// uses only the metadata from the new item. You can also combine the options
// described in [FileManager.ItemReplacementOptions] using the C-bitwise OR
// operator.
// //
...
// [ItemReplacementDirectory] as the search path directory, to get a temporary
// URL on the destination’s volume that is suitable for use with this
// method.
// 
...
// method returns [true], or if the delegate does not implement the
// appropriate methods, the file manager proceeds to copy the file or
// directory. If there is an error copying an item, the file manager may also
// call the delegate’s
...
// If the delegate method returns [true], or if the delegate does not
// implement the appropriate methods, the file manager moves the file. If
// there is an error moving one out of several items, the file manager may
// also call the delegate’s
// [FileManagerShouldProceedAfterErrorMovingItemAtURLToURL] or
...
// # Return Value
// 
// [true] if the item is targeted for iCloud storage or [false] if it is not.
// This method also returns [false] if no item exists at `url`.
//
// [false]: https://developer.apple.com/documentation/Swift/false
// [true]: https://developer.apple.com/documentation/Swift/true
//
...
// method with a value of [true] for its `flag` parameter. This method does
// not reflect whether the file has actually been uploaded to any iCloud
// servers. To determine a file’s upload status, check the
...
// directory before calling this method. When you specify [true] for the
// `flag` parameter, this method attempts to move the file or directory to the
// cloud and returns [true] if it is successful. Calling this method also
// notifies your file presenter of the new location of the file so that your
// app can continue to operate on it.
//
// [true]: https://developer.apple.com/documentation/Swift/true
//
...
// pause fails with an [FileWriteUnknownError], with an underlying error that
// has domain [NSPOSIXErrorDomain] and code [EBUSY]. If the pause fails, wait
// for the state to stabilize before retrying. Pausing also fails with
...
// 
// In most situations, the [FileManagerResumeSyncBehaviorPreserveLocalChanges]
...
// with [FileWriteUnknownError], with the underlying error of
// [serverUnreachable].
//
// [featureUnsupported]: https://developer.apple.com/documentation/Foundation/CocoaError/featureUnsupported
...
// [FileReadUnknownError], with the underlying error of [serverUnreachable].
//
// [serverUnreachable]: https://developer.apple.com/documentation/FileProvider/NSFileProviderError/serverUnreachable
//
...
// changes, uploading fails with [FileWriteUnknownError], with an underlying
...
// 
// If the device isn’t connected to the network, the call may fail with
// [FileWriteUnknownError], with the underlying error of [serverUnreachable].
//
...
// # Return Value
// 
// [true] if a file at the specified path exists, or [false] if the file does
...
// more parent directories are inaccessible, this method returns [false]. If
// the final element in `path` specifies a symbolic link, this method
// traverses the link and returns [true] or [false] based on the existence of
// the file at the link destination.
//
// [false]: https://developer.apple.com/documentation/Swift/false
// [true]: https://developer.apple.com/documentation/Swift/true
//
...
// [false]. If `path` doesn’t exist, this value is undefined upon return.
...
// # Return Value
// 
// [true] if a file at the specified path exists, or [false] if the file’s
...
func (f FileManager) FileExistsAtPathIsDirectory(path string, isDirectory bool) bool {
	rv := objc.Send[bool](f.ID, objc.Sel("fileExistsAtPath:isDirectory:"), objc.String(path), isDirectory)
	return rv
}
...
// # Return Value
// 
// [true] if the current process has execute privileges for the file at
// `path`; otherwise [false] if the process does not have execute privileges
// or the existence of the file could not be determined.
//
// [false]: https://developer.apple.com/documentation/Swift/false
// [true]: https://developer.apple.com/documentation/Swift/true
//
...
// occurs. The handler block should return [true] if you want the enumeration
// to continue or [false] if you want the enumeration to stop. The block takes
