...
// its main task or false if it is not.
//
...
// [NSQualityOfServiceDefault] and you should leave that value in place
// whenever possible. When changing the service level, use the minimum level
// that is appropriate for executing the corresponding task. For example, if
// the user initiates a task and is waiting for it to finish, assign the value
// [NSQualityOfServiceUserInteractive] to this property. The system may give
// the operation a higher service level to the operation if the resources are
