...
// # Overview
//
// Each process has a single, shared [NSProcessInfo] object known as a process
// information agent that can return information such as arguments,
// environment variables, host name, and process name. The
// [NSProcessInfoClass.ProcessInfo] class method returns the shared agent for
// the current process. For example, the following line returns the
...
// following methods to manage activities that give hints to the system that
...
//
// - User-initiated activities are explicitly started by the user. Examples
// include exporting or downloading a user-specified file. - Background
// activities perform the normal operations of your application and aren’t
// explicitly started by the user. Examples include autosaving, indexing, and
...
//
// Thermal state indicates the level of heat generated by logic components as
// they run apps. As the thermal state increases, the system decreases heat by
// reducing the speed of the processors. Optimize your app’s performance by
// monitoring the thermal state and reducing system usage as the thermal state
// increases. Query the current state with [NSProcessInfo.ThermalState] to
// determine if your app needs to reduce system usage. You can register the
...
func (p ProcessInfo) ProcessIdentifier() int32 {
	rv := objc.Send[int32](p.ID, objc.Sel("processIdentifier"))
	return rv
}
...
// appropriate for displaying to the user. This string is not appropriate for
// parsing.
//
