...
//
// - [NSStream.Open] and [NSStream.Close]
//
// Implement [NSStream.Open] to open the stream for reading or writing and
// make the stream available to the client directly or, if the stream object
// is scheduled on a run loop, to the delegate. Implement [NSStream.Close] to
// close the stream and remove the stream object from the run loop, if
// necessary. A closed stream should still be able to accept new properties
// and report its current properties. Once you close a stream, you can’t
// reopen it.
//
// - [NSStream.Delegate]
//
// Return and set the delegate. By a default, a stream object must be its own
// delegate; so a [NSStream.Delegate] message with an argument of `nil` should
// restore this delegate. Don’t retain the delegate to prevent retain
// cycles.
//
// To learn about delegates and delegation, read “Delegation” in Cocoa
// Fundamentals Guide.
//
// - [NSStream.ScheduleInRunLoopForMode] and
// [NSStream.RemoveFromRunLoopForMode]
//
// Implement [NSStream.ScheduleInRunLoopForMode] to schedule the stream object
// on the specified run loop for the specified mode. Implement
// [NSStream.RemoveFromRunLoopForMode] to remove the object from the run loop.
// See the documentation of the [NSRunLoop] class for details. Once the stream
// object for an open stream is scheduled on a run loop, it is the
// responsibility of the subclass as it processes stream data to send
// [StreamHandleEvent] messages to its delegate.
//
// - [NSStream.PropertyForKey] and [NSStream.SetPropertyForKey]
//
...
//
// - [NSStream.StreamStatus] and [NSStream.StreamError]
//
// Implement [NSStream.StreamStatus] to return the current status of the
// stream as a [Stream.Status] constant; you may define new [Stream.Status]
// constants, but be sure to handle the system defined constants properly.
// Implement [NSStream.StreamError] to return an [NSError] object representing
// the current error. You might decide to return a custom [NSError] object
