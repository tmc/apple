...
func (o NSURLSessionTaskDelegateObject) URLSessionTaskDidReceiveChallengeCompletionHandler(session INSURLSession, task INSURLSessionTask, challenge INSURLAuthenticationChallenge, completionHandler NSURLSessionAuthChallengeDispositionURLCredentialHandler) {
...
func (o NSURLSessionTaskDelegateObject) URLSessionTaskWillBeginDelayedRequestCompletionHandler(session INSURLSession, task INSURLSessionTask, request INSURLRequest, completionHandler NSURLSessionDelayedRequestDispositionURLRequestHandler) {
...
func (o NSURLSessionTaskDelegateObject) URLSessionDidReceiveChallengeCompletionHandler(session INSURLSession, challenge INSURLAuthenticationChallenge, completionHandler NSURLSessionAuthChallengeDispositionURLCredentialHandler) {
	...
	if config.URLSessionTaskDidCompleteWithError != nil {
		...
		methods = append(methods, objc.MethodDef{
			...
			Fn: func(self objc.ID, _cmd objc.SEL, sessionID objc.ID, taskID objc.ID, error_ID objc.ID) {
				...
				defer func() {
					if !_delegateDone {
						objc.NoteDelegatePanic("NSURLSessionTaskDelegate", "URLSession:task:didCompleteWithError:")
					}
				}()
	...
	if config.URLSessionTaskDidSendBodyDataTotalBytesSentTotalBytesExpectedToSend != nil {
		...
		methods = append(methods, objc.MethodDef{
			...
			Fn: func(self objc.ID, _cmd objc.SEL, sessionID objc.ID, taskID objc.ID, bytesSent int64, totalBytesSent int64, totalBytesExpectedToSend int64) {
				...
				defer func() {
					if !_delegateDone {
						objc.NoteDelegatePanic("NSURLSessionTaskDelegate", "URLSession:task:didSendBodyData:totalBytesSent:totalBytesExpectedToSend:")
					}
				}()
				session := NSURLSessionFromID(sessionID)
				task := NSURLSessionTaskFromID(taskID)
	...
	if config.URLSessionTaskIsWaitingForConnectivity != nil {
		...
		methods = append(methods, objc.MethodDef{
			...
			Fn: func(self objc.ID, _cmd objc.SEL, sessionID objc.ID, taskID objc.ID) {
				...
				defer func() {
					if !_delegateDone {
						objc.NoteDelegatePanic("NSURLSessionTaskDelegate", "URLSession:taskIsWaitingForConnectivity:")
					}
				}()
				session := NSURLSessionFromID(sessionID)
				task := NSURLSessionTaskFromID(taskID)
	...
	if config.URLSessionTaskDidFinishCollectingMetrics != nil {
		...
		methods = append(methods, objc.MethodDef{
			...
			Fn: func(self objc.ID, _cmd objc.SEL, sessionID objc.ID, taskID objc.ID, metricsID objc.ID) {
				...
				defer func() {
					if !_delegateDone {
						objc.NoteDelegatePanic("NSURLSessionTaskDelegate", "URLSession:task:didFinishCollectingMetrics:")
					}
				}()
				session := NSURLSessionFromID(sessionID)
				task := NSURLSessionTaskFromID(taskID)
	...
	if config.URLSessionDidCreateTask != nil {
		...
		methods = append(methods, objc.MethodDef{
			...
			Fn: func(self objc.ID, _cmd objc.SEL, sessionID objc.ID, taskID objc.ID) {
				...
				defer func() {
					if !_delegateDone {
						objc.NoteDelegatePanic("NSURLSessionTaskDelegate", "URLSession:didCreateTask:")
					}
				}()
				session := NSURLSessionFromID(sessionID)
				task := NSURLSessionTaskFromID(taskID)
	...
	if config.URLSessionTaskDidReceiveInformationalResponse != nil {
		...
		methods = append(methods, objc.MethodDef{
			...
			Fn: func(self objc.ID, _cmd objc.SEL, sessionID objc.ID, taskID objc.ID, responseID objc.ID) {
				...
				defer func() {
					if !_delegateDone {
						objc.NoteDelegatePanic("NSURLSessionTaskDelegate", "URLSession:task:didReceiveInformationalResponse:")
					}
				}()
				session := NSURLSessionFromID(sessionID)
				task := NSURLSessionTaskFromID(taskID)
