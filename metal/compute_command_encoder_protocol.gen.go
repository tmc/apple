...
//
// usage: How the compute pass can access data, including [ResourceUsageRead] and
// [ResourceUsageWrite] permission.
// 
...
// 
// This method applies the [ResourceUsageRead] resource usage option to all of
// the resources within the `heaps`, except for textures. The method ignores
// any texture that has [TextureUsageRenderTarget], [TextureUsageShaderWrite],
// or both in its [Usage] property. For all other textures in each of the
// `heaps`, the method optimizes each texture’s memory layout for rendering
// with a sampler. However, your kernels can’t read from those textures by
// calling this method because the texture needs a different memory layout
// that’s suitable for reading.
// 
...
// [ResourceUsageRead], overriding previous calls to [UseResourceUsage].
// 
...
// 
// This method applies the [ResourceUsageRead] resource usage option to all of
// the resources within `heap`, except for textures. The method ignores any
...
// However, your kernels can’t read from those textures by calling this
...
// to [ResourceUsageWrite] by passing it to [UseResourcesCountUsage] after
// calling this method. However, you can’t reverse the call order because
// this method resets the usage for all resources within `heap` to
// [ResourceUsageRead], overriding previous calls to [UseResourceUsage].
// 
