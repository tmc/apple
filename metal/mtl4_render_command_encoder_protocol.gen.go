...
// aligned to 2 bytes if the `indexType` format is [MTLIndexTypeUInt16], and
// aligned to 4 bytes if the format is [MTLIndexTypeUInt32].
//
...
// `indexType` format is [MTLIndexTypeUInt16], and a multiple of 4 if the
// format is [MTLIndexTypeUInt32]. If this draw call causes Metal to read
// indices at or beyond the `indexBufferLength`, Metal continues to execute
// them assigning a value of `0` to the `vertex_id` attribute.
//
// # Discussion
//
...
// [MTLIndexTypeUInt16] or [MTLIndexTypeUInt32].
//
...
// responsible for ensuring this this size is a multiple of 2 if the
// `indexType` format is [MTLIndexTypeUInt16], and a multiple of 4 if the
// format is [MTLIndexTypeUInt32]. Metal disregards this value and assigns `0`
// to the `vertex_id` attribute for all primitives that require loading
// indices at a byte offset of `indexBufferLength` or greater.
//
// instanceCount: An integer that represents the number of times the command draws
...
// [MTLRenderStageMesh] may include [MTLRenderStageVertex].
//
