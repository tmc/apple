...
func (e *unavailableSymbolError) Error() string {
	...
	if e.introduced != "" {
		return fmt.Sprintf("remotecoreml: symbol %s unavailable on this system (introduced in macOS %s)", e.symbol, e.introduced)
	}
	return fmt.Sprintf("remotecoreml: symbol %s unavailable on this system", e.symbol)
}
...
func symbolCallError(name, introduced string, err error) error {
	...
	if frameworkHandle == 0 {
		return fmt.Errorf("remotecoreml: symbol %s unavailable because the framework could not be loaded", name)
	}
...
func registerFunc(fptr any, errDst *error, handle uintptr, name, introduced string) {
	...
	defer func() {
		if r := recover(); r != nil {
			*errDst = fmt.Errorf("remotecoreml: register symbol %s: %v", name, r)
		}
	}()
...
// SymbolAddress returns the address of name in remotecoreml, whether or not
...
func SymbolAddress(name string) (uintptr, error) {
	if frameworkHandle == 0 {
		return 0, fmt.Errorf("remotecoreml: symbol %s unavailable because the framework could not be loaded", name)
	}
...
// BindFunc binds the remotecoreml symbol name into fptr, which must be a
...
func BindFunc(fptr any, name string) (err error) {
	...
	defer func() {
		if r := recover(); r != nil {
			err = fmt.Errorf("remotecoreml: bind symbol %s: %v", name, r)
		}
	}()
	purego.RegisterFunc(fptr, sym)
	return nil
