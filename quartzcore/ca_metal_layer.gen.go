...
// # Discussion
//
// If the value is true (the default), the [CAMetalLayer] class allocates its
// [MTLTexture] objects with only the [renderTarget] usage flag. Core
...
//
// Setting this value to true makes the layer draw its contents synchronously,
// using whichever Core Animation transaction is current at the time you call
// the drawable’s [present()] method. To ensure that a transaction is
// available when you schedule the drawable to be presented, first commit the
// command buffer containing your Metal rendering commands. Then, call its
// [waitUntilScheduled()] method to synchronously wait until the command queue
// schedules the command buffer to execute on the GPU. Finally, call the
// drawable’s [present()] method.
//
