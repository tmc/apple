...
func TestRDMACall3ProductionLink(t *testing.T) {
	// The fixture is under testdata so that it stays out of ./... and out of
	// cmd/: it is a test input, not a command anyone should install.
	binary := filepath.Join(t.TempDir(), "call3smoke")
	build := exec.Command("go", "build", "-o", binary, "./rdma/testdata/call3smoke")
	build.Dir = ".."
