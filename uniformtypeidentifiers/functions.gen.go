...
func registerFunc(fptr any, handle uintptr, name string) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "warning: UniformTypeIdentifiers: symbol %s not found\n", name)
		return
	}
	defer func() {
		if r := recover(); r != nil {
			fmt.Fprintf(os.Stderr, "warning: UniformTypeIdentifiers: symbol %s registration skipped: %v\n", name, r)
		}
	}()
