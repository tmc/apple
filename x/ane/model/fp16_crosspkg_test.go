...
// TestFP16CrossPackageHashesModel reports this package's scalar fp16 helper hashes.
func TestFP16CrossPackageHashesModel(t *testing.T) {
	if got := fp16ReadPopulation(fp16ToFloat32); got != fp16WantReadHash {
		t.Errorf("pkg model fp16->float32 hash = %#016x, want %#016x (scalar copies have drifted)", got, uint64(fp16WantReadHash))
	} else {
		t.Logf("pkg model fp16->float32 hash = %#016x (agrees with all other copies)", got)
	}
	if got := fp16WritePopulation(float32ToFP16); got != fp16WantWriteHash {
		t.Errorf("pkg model float32->fp16 hash = %#016x, want %#016x (scalar copies have drifted)", got, uint64(fp16WantWriteHash))
	} else {
		t.Logf("pkg model float32->fp16 hash = %#016x (agrees with all other copies)", got)
	}
}
