...
type Server struct {
	...
	objs    sync.Map // Item -> objc.ID (FSItem), the reverse of items
...
// newItem returns the FSItem representing item, allocating one only the first
// time item is seen.
//
// The same file must come back as the same FSItem every time. FSKit correlates
// items by object identity, not by any id inside them: overItem is documented
// as "the file system object if the destination exists, as discovered in a
// prior lookup", and reclaimItem: retires one specific object. Handing out a
// fresh FSItem per lookup left FSKit unable to match a looked-up item against
// the one an operation names, so it declined to dispatch the operations that
// need that match — removeItem: was never delivered (unlink reported success
// and removed nothing) and a rename onto an existing name failed with ENOENT.
// Both worked as soon as identity was stable.
//
// This only holds if the Volume returns a stable Item per file; a Volume that
// mints a new Item per lookup defeats it, since Item is the key here.
func (s *Server) newItem(v Volume, item Item) objc.ID {
	if obj, ok := s.objs.Load(item); ok {
		return obj.(objc.ID)
	}
	obj := objc.Send[objc.ID](objc.ID(s.classes.Item), objc.Sel("alloc"))
	obj = objc.Send[objc.ID](obj, objc.Sel("init"))
	if existing, loaded := s.objs.LoadOrStore(item, obj); loaded {
		// Lost a race: release the extra allocation and use the winner.
		objc.Send[struct{}](obj, objc.Sel("release"))
		return existing.(objc.ID)
	}
